...
export class UsersService {
    ...
    try {
      ...
      if (!user) {
        throw new BadRequestException(
          `Người dùng (Email: ${email}) không tồn tại trong hệ thống!`,
        );
      }
  ...
  async create(createUserDto: CreateUserDto): Promise<UserResponse | null> {
    try {
      const part = await this.partRepository.existsBy({
        id: createUserDto.partId,
      });
      if (!part) {
        throw new BadRequestException(
          `Phòng ban (ID: ${createUserDto.partId}) không tồn tại trong hệ thống!`,
        );
      }
  ...
  async update(
    ...
    try {
      ...
      if (!user) {
        throw new BadRequestException(
          `Người dùng (ID: ${id}) không tồn tại trong hệ thống!`,
        );
      }
      if (updateUserDto.partId) {
        const part = await this.partRepository.existsBy({
          id: updateUserDto.partId,
        });
        if (!part) {
          throw new BadRequestException(
            `Phòng ban (ID: ${updateUserDto.partId}) không tồn tại trong hệ thống!`,
          );
        }
      }
