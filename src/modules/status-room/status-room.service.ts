...
export class StatusRoomService {
  ...
  async create(
    createStatusRoomDto: CreateStatusRoomDto,
  ...
  async findAll(): Promise<StatusRoomResponse[]> {
    try {
      ...
      return statusRooms.map(
        (statusRoom) => new StatusRoomResponse(statusRoom),
      );
    } catch (error) {
      throw new InternalServerErrorException((error as Error).message);
    }
  ...
  async findOne(id: string): Promise<StatusRoomResponse> {
    try {
      const statusRoom = await this.statusRoomRepository.findOne({
        where: { id },
      });
      if (!statusRoom) {
  ...
  async update(
    id: string,
    updateStatusRoomDto: UpdateStatusRoomDto,
  ): Promise<ApiResponse<null>> {
    try {
      const statusRoom = await this.statusRoomRepository.findOne({
        where: { id },
